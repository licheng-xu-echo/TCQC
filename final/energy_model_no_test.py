...
def main():
    ...
    print(f'Train: {len(split_idx["train"])+len(split_idx["test"])}, Valid: {len(split_idx["valid"])}, test: {len(split_idx["test"])}')
    train_dataset, valid_dataset = dataset[torch.cat([split_idx['train'],split_idx['test']])], dataset[split_idx['valid']]
    if len(split_idx['test']) == 0:
        test_dataset = valid_dataset
    else:
        test_dataset = dataset[split_idx['test']][:32]
