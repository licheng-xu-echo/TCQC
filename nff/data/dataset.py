...
from .parallel import (featurize_parallel, NUM_PROCS,
                               add_e3fp_parallel, add_kj_ji_parallel,
                               add_bond_idx_parallel)
from .features import ATOM_FEAT_TYPES, BOND_FEAT_TYPES, ATOM_ENERGY, HA2KCAL
from .features import add_morgan as external_morgan
from .features import featurize_rdkit as external_rdkit
from .graphs import (get_bond_idx, reconstruct_atoms,
...
class QMDataset(InMemoryDataset):
    def __init__(self, root, name='QMA_round1_train_230725_0.npy', raw_data=[],transform=None, pre_transform=None,train=True):
        self.name = name
        self.train = train
    ...
    def processed_file_names(self):
        return [f'{self.name[:-4]}.pt']
        # return ['QMA_round1_train_proc_energy_drp_high_grad.pt']
    ...
    def process(self):
        if len(self.raw_data) != 0:
            ...
            raw_data = np.load(osp.join(self.root, self.raw_file_names),allow_pickle=True)
        data_list = []
        for i in tqdm(range(len(raw_data))):
            R_i = torch.tensor(raw_data[i]['coordinates'],dtype=torch.float32)
            z_i = torch.tensor(raw_data[i]['elements'],dtype=torch.int64)
            nxyz = torch.cat([z_i.view(-1,1),R_i],dim=1)
            if self.train:
                E_i = torch.tensor(raw_data[i]['energy'],dtype=torch.float32) - sum([ATOM_ENERGY['B3LYP/DEF2TZVP'][int(ele)] for ele in z_i])*HA2KCAL
                if E_i > 0:
                    continue
                E_grad = -torch.tensor(raw_data[i]['force'].reshape(-1,3),dtype=torch.float32)
                data = Data(pos=R_i, z=z_i, energy=E_i, y=E_i, energy_grad=E_grad, nxyz=nxyz)
            else:
                data = Data(pos=R_i, z=z_i, nxyz=nxyz)
            data_list.append(data)
        data, slices = self.collate(data_list)
        print(f'[INFO] {len(data_list)} Saving...')
...
class QMMultiDataset(InMemoryDataset):
    def __init__(self, root, name, transform=None, pre_transform=None,train=True):
        self.name = name  ## 保存的数据pt文件的名字
        self.train = train
        super().__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def raw_file_names(self):
        npy_files = sorted(glob.glob(f'{self.root}/QMB_*.npy'))
        return [os.path.basename(item) for item in npy_files if not 'drp_high' in item]
    #返回process方法所需的保存文件名。保存的数据集名字和列表里的一致
    @property
    def processed_file_names(self):
        return [f'{self.name[:-4]}.pt']
    #生成数据集所用的方法
    def process(self):
        ...
        for raw_file_name in self.raw_file_names:
            raw_data = np.load(osp.join(self.root, raw_file_name),allow_pickle=True)
            for i in tqdm(range(len(raw_data))):
                R_i = torch.tensor(raw_data[i]['coordinates'],dtype=torch.float32)
                z_i = torch.tensor(raw_data[i]['elements'],dtype=torch.int64)
                nxyz = torch.cat([z_i.view(-1,1),R_i],dim=1)
        ...
        print(f'[INFO] {len(data_list)} Saving...')
        torch.save((data, slices), self.processed_paths[0])
        np.save(self.root+'/'+self.name,np.array([]))   ## placeholder
    def get_idx_split(self, data_size, train_size, valid_size, seed):
        ids = shuffle(range(data_size), random_state=seed)
        train_idx, val_idx, test_idx = torch.tensor(ids[:train_size]), torch.tensor(ids[train_size:train_size + valid_size]), torch.tensor(ids[train_size + valid_size:])
        split_dict = {'train':train_idx, 'valid':val_idx, 'test':test_idx}
...
class NewQMDataset(Dataset):
    ...
    @property
    def raw_file_names(self):
        return ['QMB_round2_train_proc_energy_drp_high0.npy','QMB_round2_train_proc_energy_drp_high1.npy',
                'QMB_round2_train_proc_energy_drp_high2.npy','QMB_round2_train_proc_energy_drp_high3.npy']
    
    #返回process方法所需的保存文件名。你之后保存的数据集名字和列表里的一致
    @property
    def processed_file_names(self):
        return ['QMB_round2_train_proc_energy_drp_high0.pt','QMB_round2_train_proc_energy_drp_high1.pt',
                'QMB_round2_train_proc_energy_drp_high2.pt','QMB_round2_train_proc_energy_drp_high3.pt']
        # return ['QMA_round1_train_proc_energy_drp_high_grad.pt']
    ...
    def process(self):
        ...
        for idx,raw_path in enumerate(self.raw_paths):
            ...
            data, slices = self.collate(data_list)
            print(f'[INFO] {len(data_list)} Saving...')
            torch.save(data, osp.join(self.processed_dir, self.processed_file_names[idx]))
    ...
    def len(self):
        ...
        tot_len = 0
        if self.tot_slices == []:
            self.dataset_right = [0]
            for data_f in self.processed_file_names:
                data_path = osp.join(self.processed_dir,data_f)
                data,slices = torch.load(data_path)
                self.tot_data.append(data)
                self.tot_slices.append(slices)
                self.dataset_right.append(self.dataset_right[-1]+data.y.shape[0])
                tot_len += len(data.y)
        else:
            for data in self.tot_data:
                tot_len += len(data)
        self.cumulative_sizes = self.cumsum(self.tot_data)
        return tot_len
    def __len__(self):
        return self.len()
    def get(self, idx):
        ...
        return separate(
            cls=self.tot_data[dataset_idx-1].__class__,
            batch=self.tot_data[dataset_idx-1],
            idx=sample_idx,
            slice_dict=self.tot_slices[dataset_idx-1],
            decrement=False,
        )
    def __getitem__(self,idx):
        return self.get(idx)
class QMForceDataset(InMemoryDataset):
    def __init__(self, root, name='QMA_round1_train_230725_0.npy',transform=None, grad_th=100, pre_transform=None,train=True):
        self.name = name
        self.train = train
        ...
        for i in tqdm(range(len(raw_data))):
            ...
            if self.train:
                ...
                if E_i > 0:
                    continue
                E_grad = -torch.tensor(raw_data[i]['force'].reshape(-1,3),dtype=torch.float32)
                if torch.max(E_grad) > self.grad_th or torch.min(E_grad) < -self.grad_th:
                    continue
                data = Data(pos=R_i, z=z_i, energy=E_i, y=E_i, energy_grad=E_grad, nxyz=nxyz)
            else:
                data = Data(pos=R_i, z=z_i, nxyz=nxyz)
