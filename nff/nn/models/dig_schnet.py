...
'''
From https://github.com/divelab/DIG/blob/dig-stable/dig/threedgraph/method/schnet/schnet.py
'''
...
class DIGSchNet(torch.nn.Module):
    r"""
    ...
    """
    def __init__(self, energy_and_force=False, cutoff=10.0, num_layers=6, hidden_channels=128, out_channels=1, num_filters=128, num_gaussians=50,ret_res_dict=True):
        super(DIGSchNet, self).__init__()
    ...
    def forward(self, batch_data):
        ...
        results = {'energy':u}
        if self.energy_and_force:
            energy_grad = grad(outputs=u, inputs=batch_data['pos'], grad_outputs=torch.ones_like(u),create_graph=True,retain_graph=True)[0]
            results['energy_grad'] = energy_grad
        if not self.energy_and_force and not self.ret_res_dict:
            return u
        return results
